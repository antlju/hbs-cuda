...
__global__ void kernel(Real *f, Real *df)
{
	...
	if (j < NN && k < NN)
	{
		...
		B[i][bj][bk] = f[fIdx(i,j,k)];
		__syncthreads();
			
		df[fIdx(i,j,k)] = B[i][bj][bk];
...
Int main()
{
	...
	//dim3 tpb(NY_TILE,NZ_TILE); //1024 max threads per block on gtx 850M
	dim3 tpb(NZ_TILE,NY_TILE,NX_TILE); //2*2*128 = 512 threads
