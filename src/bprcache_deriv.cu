...
__global__ void kernel(const Real *f, Real *df, const Real dx)
{
	...
	if (j < NY && k < NZ)
	{
		for (Int i=0;i<NX;i++)
		{
			...
			/// Load shared memory 
			fs[bj][bk] = f[fIdx(i+2,j,k)];

			/// If at yz-tile edges assign ghost points
			if (bj == NG)
			{
				fs[bj-1][bk] = f[fIdx(i+2,j-1,k)];
				fs[bj-2][bk] = f[fIdx(i+2,j-2,k)];
				fs[bj+NY_TILE][bk] = f[fIdx(i+2,j+NY_TILE,k)];
				fs[bj+NY_TILE+1][bk] = f[fIdx(i+2,j+NY_TILE+1,k)];
			}
			if (bk == NG)
			{
				fs[bj][bk-1] = f[fIdx(i+2,j,k-1)];
				fs[bj][bk-2] = f[fIdx(i+2,j,k-2)];
				fs[bj][bk+NZ_TILE] = f[fIdx(i+2,j,k+NZ_TILE)];
				fs[bj][bk+NZ_TILE+1] = f[fIdx(i+2,j,k+NZ_TILE+1)];
			}
			__syncthreads();
			...
			df[fIdx(i,j,k,vi)] = delz(B,1.0/dx,bi,vi); /// bi should be 0 always!
